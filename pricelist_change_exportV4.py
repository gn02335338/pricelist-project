...
def load_pricelist_file(file_path, log_func=None):
    ...
    for sheet_name, df in sheets.items():
        ...
        if not all(col in df.columns for col in required):
            ...
            continue
        df = df[required]
        df = clean_numeric_column(df, 'Price')  # 排除 Price 非數值的列
        if df.empty:
            print(f"Skip sheet {sheet_name}: no valid Price rows")
            continue
...
def upload_pricelist_to_snowflake(file_path, conn_params=SNOWFLAKE_CONFIG, prefix='PRICELIST', log_func=print):
    ...
    # --- 關鍵修正：先 strip 再轉 NaN ---
    df['Price'] = df['Price'].astype(str).str.strip().replace('', np.nan)
    df['Price'] = pd.to_numeric(df['Price'], errors='coerce')
    df = df.dropna(subset=['Price'])

    # 再次確認非空
    if df.empty:
        raise ValueError("No valid data to upload after cleaning Price")

    # 上傳至 Snowflake
    table_name = f"{prefix}_{time.strftime('%Y%m%d_%H%M%S')}_{SNOWFLAKE_CONFIG['user']}"
...
def run_compare(old_file, new_file, output_file, log_func=print):
    ...
    def check_issues(df, sheet_name, which):
        def log_issue(row, issue_type):
            if pd.isna(row['Module']) or pd.isna(row['Description']) or row['Description'] == 'nan':
                return
            issues.append({
                'Sheet': sheet_name,
                'Module': row['Module'],
